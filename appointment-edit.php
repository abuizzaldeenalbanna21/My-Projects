...
// جلب بيانات الموعد الحالي للتعديل
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($db->insert("appointments", $data)) {
        ...
        $error = "An error occurred while adding the appointment. Please try again.";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_appointment'])) {
    ...
    if ($db->update("appointments", $updateData, $where)) {
        header("Location: appointment.php");
        exit;
    } else {
        $error = "An error occurred while updating the appointment. Please try again.";
    }
}
?>
<!DOCTYPE html>
                            ...
                            <form method="POST" action="">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>Date</label>
                                            <input type="text" name="date" class="form-control" id="appointment-date" required value="<?= $appointment ? htmlspecialchars($appointment['date']) : '' ?>">
                                        </div>
                                    </div>
                                    <script>
                                    ...
                                    // عند تحميل الصفحة، عيّن اليوم تلقائياً إذا كان هناك موعد
                                    window.addEventListener('DOMContentLoaded', function() {
                                        <?php if ($appointment): ?>
                                            var dateVal = "<?= htmlspecialchars($appointment['date']) ?>";
                                            if (dateVal) {
                                                var dateObj = new Date(dateVal);
                                                var dayName = days[dateObj.getDay()];
                                                document.getElementById('appointment-day').value = dayName;
                                            }
                                        <?php endif; ?>
                                    });
                                    </script>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>Day</label>
                                            <input type="text" name="day" class="form-control" id="appointment-day" required readonly value="<?= $appointment ? htmlspecialchars($appointment['day']) : '' ?>">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select name="time" class="form-control" id="appointment-time" required>
                                                <?php if ($appointment): ?>
                                                    <option value="<?= htmlspecialchars($appointment['time']) ?>" selected><?= htmlspecialchars($appointment['time']) ?></option>
                                                <?php endif; ?>
                                    ...
                                    <script>
                                        document.getElementById('appointment-date').addEventListener('change', function () {
                                            ...
                                            fetch('get_available_times.php?date=' + selectedDate + '&appointment_id=<?= $appointment_id ?>')
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="treatment" class="form-control" required>
                                                ...
                                                <?php foreach ($treatments as $treatment): ?>
                                                    <option value="<?= $treatment['id'] ?>" <?= ($appointment && $appointment['treatment_id'] == $treatment['id']) ? 'selected' : '' ?>>
